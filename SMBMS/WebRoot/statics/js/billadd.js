...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/bill/providerlist",//请求的url
	...
	billCode.on("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/bill/checkCode",//请求的url
			data:{billCode:billCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data.billCode == "exist"){//账号已存在，错误提示
					validateTip(billCode.next(),{"color":"red"},imgNo+ " 该编码已存在",false);
				}else if(data.billCode == "noexist"){//账号可用，正确提示
					validateTip(billCode.next(),{"color":"green"},imgYes+" 该编码可以使用",true);
				}else{
					validateTip(billCode.next(),{"color":"red"},imgNo+" 编码不能为空，请重新输入",false);
				}
			},
	...
	productDesc.on("focus",function(){
		...
		if(productDesc.val() != null && productDesc.val() != ""){
			...
			validateTip(productDesc.next(),{"color":"red"},imgNo+" 描述不能为空，请重新输入",false);
		}
